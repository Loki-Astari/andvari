...
function cleanDirectory(appData, dir, files) {
    return new Promise(function(resolve, reject) {
        var rimraf = require('rimraf');
        rimraf(dir + files, function (err) {
            if (err) {reject(err);}
            else     {resolve();}
        });
    });
...
function buildSpecifiedDir(appData, root, src, dir) {
    var mkdirp = require('mkdirp');
    dir = dir.replace(src, root);
...
function buildSiteDir(appData, src, dir) {
    buildSpecifiedDir(appData, appData.app.dir.public, src, dir);
}
function buildSkelLayoutDir(appData, src, dir) {
    buildSpecifiedDir(appData, appData.app.dir.skelLayout, src, dir);
}
...
var util = {
    ...
    buildSpecifiedDir:  buildSpecifiedDir,
    buildSiteDir:       buildSiteDir,
    buildSkelLayoutDir: buildSkelLayoutDir,
