...
function processPages(appData) {
    ...
    console.log(appData.site.categories);
    for(var loop = 0; loop < appData.site.categories.length; loop++) {
        console.log('Index: ' + loop + '  Name: ' + appData.site.categories[loop]);
        var fileName = '/categories/' + appData.site.categories[loop] + '/index.html';
        var filePath = fileName.replace(baseDir + '/', '');
        var baseDir = appData.app.dir.appRoot;
