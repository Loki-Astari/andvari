...
*/
        ...
        var fs = require('fs');
        var ejs = require('ejs');
        html = ejs.renderFile(appData.app.dir.skelLayout + 'page.ejs', Object.assign({}, appData, {page: page}), function(err, result) {
            if (err) {reject(err);}
            else {
...
function CopyOtherFilesToDest(appData, root, page, reject, OKAction = function(){}) {
    return new Promise(function(resolve, reject) {
        var src = page.obj;
        var dst = page.obj.replace(page.dir, root);
        var fs = require('fs');
        var writer = fs.createWriteStream(dst);
        writer.on('close', OKAction);
        fs.createReadStream(src).pipe(writer);
        resolve();
    })
    .catch(err => reject(err));
}
function CopyOtherFiles(appData, page, reject, OKAction = function(){}) {
    return CopyOtherFilesToDest(appData, appData.app.dir.public, page, reject, OKAction);
}
function CopyLayoutFiles(appData, page, reject, OKAction = function(){}) {
    return CopyOtherFilesToDest(appData, appData.app.dir.skelLayout, page, reject, OKAction);
}
function buildAllFiles(appData, list, text, sub, src, wait = false) {

    return new Promise(function(resolve, reject) {

        // If wait is true
        // Then we can not resolve the promise until all copies are complete.
        // So we pass a function to the copy routine. After each copy is complete
        // we get a callback on the function. When we have counted all callbacks then
        // we resolve.
        var count = 0;
        resulveAtEnd = wait  ? function(){} : function(){resolve();};
        resolveWait  = !wait ? function(){} : function(){++count; if (count == list.length){resolve();}};

        for(var loop = 0; loop < list.length; loop++) {
            console.log('\t' + text + ': ' + src(list[loop]));
            sub(appData, list[loop], reject, resolveWait);
        }
        resulveAtEnd();
    })
    .catch(err => {throw err;});
...
function processPages(appData) {
    return new Promise(function(resolve, reject) {
        ...
        .then(() => buildAllFiles(appData, appData.layouts[0], 'Build Layout Global', CopyLayoutFiles, (page) => page.obj, true))
        .then(() => buildAllFiles(appData, appData.layouts[1], 'Build Layout Theme', CopyLayoutFiles, (page) => page.obj, true))
        .then(() => buildAllFiles(appData, appData.layouts[2], 'Build Layout App', CopyLayoutFiles, (page) => page.obj, true))
        .then(function() {
